    ...
    def run(self):
        ...
        for policy in self.json_array:
            if policy['type'] == 'restrictions':
                ...
                payloadIdentifierString.text = 'com.toppatch.testProfile.restrictions'
                ...
                if restrictionDict.has_key('allowAddingGameCenterFriends'):
                    allowGameCenterValue = ET.SubElement(dictSetting,str(restrictionDict['allowAddingGameCenterFriends']).lower())
                else:
                ...
                if restrictionDict.has_key('allowAppInstallation'):
                    allowAppInstallValue = ET.SubElement(dictSetting,str(restrictionDict['allowAppInstallation']).lower())
                else:
                ...
                if restrictionDict.has_key('allowAssistant'):
                    allowAssistantValue = ET.SubElement(dictSetting, str(restrictionDict['allowAssistant']).lower())
                else:
                ...
                if restrictionDict.has_key('allowAssistantWhileLocked'):
                    allowAssistantWhileLockedValue = ET.SubElement(dictSetting, str(restrictionDict['allowAssistantWhileLocked']).lower())
                else:
                    allowAssistantWhileLockedValue = ET.SubElement(dictSetting, 'true')


                allowCamera = ET.SubElement(dictSetting,'key')
                allowCamera.text = 'allowCamera'
                if restrictionDict.has_key('allowCamera'):
                    allowCameraValue = ET.SubElement(dictSetting,str(restrictionDict['allowCamera']).lower())
                else:
                ...
                if restrictionDict.has_key('allowCloudBackup'):
                    allowCloudBackupValue = ET.SubElement(dictSetting, str(restrictionDict['allowCloudBackup']).lower())
                else:
                ...
                if restrictionDict.has_key('allowCloudDocumentSync'):
                    allowDocSyncValue = ET.SubElement(dictSetting, str(restrictionDict['allowCloudDocumentSync']).lower())
                else:
                ...
                if restrictionDict.has_key('allowDiagnosticSubmission'):
                    allowDiagnosticValue = ET.SubElement(dictSetting, str(restrictionDict['allowDiagnosticSubmission']).lower())
                else:
                ...
                if restrictionDict.has_key('allowExplicitContent'):
                    allowExplicitValue = ET.SubElement(dictSetting, str(restrictionDict['allowExplicitContent']).lower())
                else:
                    allowExplicitValue = ET.SubElement(dictSetting, 'true')


                allowGlobalBckground = ET.SubElement(dictSetting, 'key')
                allowGlobalBckground.text = 'allowGlobalBackgroundFetchWhenRoaming'
                if restrictionDict.has_key('allowGlobalBackgroundFetchWhenRoaming'):
                    allowGlobalBckgroundValue = ET.SubElement(dictSetting, str(restrictionDict['allowGlobalBackgroundFetchWhenRoaming']).lower())
                else:
                    allowGlobalBckgroundValue = ET.SubElement(dictSetting, 'true')



                allowInAppPurchase = ET.SubElement(dictSetting, 'key')
                allowInAppPurchase.text = 'allowInAppPurchases'
                if restrictionDict.has_key('allowInAppPurchases'):
                    allowInAppPurchaseValue = ET.SubElement(dictSetting, str(restrictionDict['allowInAppPurchases']).lower())
                else:
                    allowInAppPurchaseValue = ET.SubElement(dictSetting, 'true')


                allowMultiplayer = ET.SubElement(dictSetting, 'key')
                allowMultiplayer.text = 'allowMultiplayerGaming'
                if restrictionDict.has_key('allowMultiplayerGaming'):
                    allowMultiplayerValue = ET.SubElement(dictSetting, str(restrictionDict['allowMultiplayerGaming']).lower())
                else:
                ...
                if restrictionDict.has_key('allowPhotoStream'):
                    allowPhotoStreamValue = ET.SubElement(dictSetting, str(restrictionDict['allowPhotoStream']).lower())
                else:
                ...
                if restrictionDict.has_key('allowSafari'):
                    allowSafariValue = ET.SubElement(dictSetting, str(restrictionDict['allowSafari']).lower())
                ...
                if restrictionDict.has_key('allowScreenShot'):
                    allowScreenshotValue = ET.SubElement(dictSetting, str(restrictionDict['allowScreenShot']).lower())
                else:
                ...
                if restrictionDict.has_key('allowUntrustedTLSPrompt'):
                    allowUntrustedTLSValue = ET.SubElement(dictSetting, str(restrictionDict['allowUntrustedTLSPrompt']).lower())
                else:
                ...
                if restrictionDict.has_key('allowVideoConferencing'):
                    allowVideoConfValue = ET.SubElement(dictSetting, str(restrictionDict['allowVideoConferencing']).lower())
                else:
                ...
                if restrictionDict.has_key('allowVoiceDialing'):
                    allowVoiceDialValue = ET.SubElement(dictSetting, str(restrictionDict['allowVoiceDialing']).lower())
                else:
                ...
                if restrictionDict.has_key('allowYouTube'):
                    allowYoutubeValue = ET.SubElement(dictSetting, str(restrictionDict['allowYouTube']).lower())
                else:
                ...
                if restrictionDict.has_key('allowiTunes'):
                    allowiTunesValue = ET.SubElement(dictSetting,str(restrictionDict['allowiTunes']).lower())
                else:
                ...
                if restrictionDict.has_key('forceEncryptedBackup'):
                    allowForceEncryptValue = ET.SubElement(dictSetting, str(restrictionDict['forceEncryptedBackup']).lower())
                else:
                ...
                if restrictionDict.has_key('forceITunesStorePasswordEntry'):
                    allowForceiTunePassValue = ET.SubElement(dictSetting, str(restrictionDict['forceITunesStorePasswordEntry']).lower())
                else:
                    allowForceiTunePassValue = ET.SubElement(dictSetting, 'true')
                ...
                if self.useSafari == True:
                    safariAcceptCookie = ET.SubElement(dictSetting, 'key')
                    safariAcceptCookie.text = 'safariAcceptCookies'
                    safariAcceptCookieString = ET.SubElement(dictSetting, 'integer')
                    if restrictionDict.has_key('safariAcceptCookies'):
                        safariAcceptCookieString.text = str(restrictionDict['safariAcceptCookies'])
                    else:
                    ...
                    if restrictionDict.has_key('safariAllowAutoFill'):
                        allowAutoFillValue = ET.SubElement(dictSetting, str(restrictionDict['safariAllowAutoFill']).lower())
                    else:
                    ...
                    if restrictionDict.has_key('safariAllowJavaScript'):
                        allowJSValue = ET.SubElement(dictSetting, str(restrictionDict['safariAllowJavaScript']).lower())
                    else:
                    ...
                    if restrictionDict.has_key('safariAllowPopups'):
                        allowPopUpsValue = ET.SubElement(dictSetting, str(restrictionDict['safariAllowPopups']).lower())
                    else:
                    ...
                    if restrictionDict.has_key('safariForceFraudWarning'):
                        allowFraudWarnValue = ET.SubElement(dictSetting, str(restrictionDict['safariForceFraudWarning']).lower())
                    else:
                        allowFraudWarnValue = ET.SubElement(dictSetting, 'false')
...
#         if self.json_dict.has_key('passcode'):
            if policy['type'] == 'passcode':
                ...
                if passcodeDict.has_key('allowSimple'):
                    allowSimple = ET.SubElement(dictSetting, 'key')
                    allowSimple.text = 'allowSimple'
                    allowSimpleValue = ET.SubElement(dictSetting, str(passcodeDict['allowSimple']).lower())
                else:
                ...
                if passcodeDict.has_key('maxFailedAttempts'):
                    maxFailedAttempts = ET.SubElement(dictSetting, 'key')
                    maxFailedAttempts.text = 'maxFailedAttempts'
                    maxFailedAttemptsString = ET.SubElement(dictSetting, 'integer')
                    maxFailedAttemptsString.text = str(passcodeDict['maxFailedAttempts'])


                if passcodeDict.has_key('maxGracePeriod'):
                    maxGracePeriod = ET.SubElement(dictSetting, 'key')
                    maxGracePeriod.text = 'maxGracePeriod'
                    maxGracePeriodString = ET.SubElement(dictSetting, 'integer')
                    maxGracePeriodString.text = str(passcodeDict['maxGracePeriod'])


                if passcodeDict.has_key('maxInactivity'):
                    ...
                    maxInactivityString.text = str(passcodeDict['maxInactivity'])


                if passcodeDict.has_key('maxPINAgeInDays'):
                    maxPINAgeInDays = ET.SubElement(dictSetting, 'key')
                    maxPINAgeInDays.text = 'maxPINAgeInDays'
                    maxPINAgeInDaysString = ET.SubElement(dictSetting, 'integer')
                    maxPINAgeInDaysString.text = str(passcodeDict['maxPINAgeInDays'])


                if passcodeDict.has_key('minComplexChars'):
                    minComplexChars = ET.SubElement(dictSetting, 'key')
                    minComplexChars.text = 'minComplexChars'
                    minComplexCharsString = ET.SubElement(dictSetting, 'integer')
                    minComplexCharsString.text = str(passcodeDict['minComplexChars'])


                if passcodeDict.has_key('minLength'):
                ...
                if passcodeDict.has_key('pinHistory'):
                ...
                if passcodeDict.has_key('requireAlphanumeric'):
                    requireAlphanumericString = ET.SubElement(dictSetting, str(passcodeDict['requireAlphanumeric']).lower())
                else:
                    requireAlphanumericString = ET.SubElement(dictSetting, 'true')
...
#         if self.json_dict.has_key('exchangeAccount'):
            if policy['type'] == 'exchangeAccount':
                ...
                if eaDict.has_key('EmailAddress'):
                    emailAddressString.text = str(eaDict['EmailAddress'])
                else:
                    emailAddressString.text = someDefaultEmail
                    self.log.e(tag, 'Email Address not sent in EmailField of Exachange Account')


                Host = ET.SubElement(dictSetting, 'key')
                Host.text = 'Host'
                HostString = ET.SubElement(dictSetting, 'string')
                if eaDict.has_key('Host'):
                    emailAddressString.text = str(eaDict['Host'])
                else:
                    HostString.text = someDefaultEmail
                    self.log.e(tag, 'Host not sent in Host of Exachange Account')


                MailNumberOfPastDaysToSync = ET.SubElement(dictSetting, 'key')
                MailNumberOfPastDaysToSync.text = 'MailNumberOfPastDaysToSync'
                MailNumberOfPastDaysToSyncString = ET.SubElement(dictSetting, 'integer')
                if eaDict.has_key('MailNumberOfPastDaysToSync'):
                    emailAddressString.text = str(eaDict['MailNumberOfPastDaysToSync'])
                else:
                ...
                if eaDict.has_key('Password'):
                    emailAddressString.text = str(eaDict['Password'])
                else:
                    PasswordString.text = ''
                    self.log.e(tag, 'Password not sent in Host of Exachange Account')
                ...
                payloadName.text = 'PayloadDisplayName'
                payloadNameString = ET.SubElement(dictSetting, 'string')
                if eaDict.has_key('PayloadDisplayName'):
                ...
                if eaDict.has_key('PreventAppSheet'):
                    PreventAppSheetString = ET.SubElement(dictSetting, str(eaDict['PreventAppSheet']).lower())
                else:
                ...
                if eaDict.has_key('PreventMove'):
                    PreventMoveString = ET.SubElement(dictSetting, str(eaDict['PreventMove']).lower())
                else:
                ...
                if eaDict.has_key('SMIMEEnabled'):
                    SMIMEEnabledString = ET.SubElement(dictSetting, str(eaDict['SMIMEEnabled']).lower())
                else:
                ...
                if eaDict.has_key('UserName'):
                    UserNameString.text = ET.SubElement(dictSetting, eaDict['UserName'])
                else:
...
#         if self.json_dict.has_key('webClip'):
            if policy['type'] == 'webClip':
                ...
                if webClipDict.has_key('FullScreen'):
                    FullScreenValue = ET.SubElement(dictSetting, str(webClipDict['FullScreen']).lower())
                else:
                ...
                if webClipDict.has_key('IsRemovable'):
                    IsRemovableValue = ET.SubElement(dictSetting, str(webClipDict['IsRemovable']).lower())
                else:
                ...
                if webClipDict.has_key('Precomposed'):
                    PrecomposedValue = ET.SubElement(dictSetting, str(webClipDict['Precomposed']).lower())
                else:
...
#         if self.json_dict.has_key('wifi'):
            if policy['type'] == 'wifi':
                ...
                for e,wifiDict in enumerate(wifiDict_list):
                    ...
                    if wifiDict.has_key('AutoJoin'):
                        AutoJoinValue = ET.SubElement(dictSetting, str(wifiDict['AutoJoin']).lower())
                    else:
                    ...
                    if wifiDict.has_key('EncryptionType'):
                        EncryptionTypeString.text = str(wifiDict['EncryptionType'])
                    else:
                    ...
                    if wifiDict.has_key('HIDDEN_NETWORK'):
                        HIDDEN_NETWORKValue = ET.SubElement(dictSetting, str(wifiDict['HIDDEN_NETWORK']).lower())
                    else:
                        HIDDEN_NETWORKValue = ET.SubElement(dictSetting, 'false')


                    Password = ET.SubElement(dictSetting, 'key')
                    Password.text = 'Password'
                    PasswordString = ET.SubElement(dictSetting, 'string')
                    if wifiDict.has_key('Password'):
                    ...
                    payloadName.text = 'PayloadDisplayName'
                    payloadNameString = ET.SubElement(dictSetting, 'string')
                    if wifiDict.has_key('SSID_STR'):
                        payloadNameString.text = 'Wi-Fi ('+ str(wifiDict['SSID_STR']) + ')'
                    else:
                    ...
                    payloadIdentifier.text = 'PayloadIdentifier'
                    payloadIdentifierString = ET.SubElement(dictSetting, 'string')
                    payloadIdentifierString.text = 'com.toppatch.wifiProfile.wifi{0}'.format(e)
                    ...
                    payloadUUID.text = 'PayloadUUID'
                    payloadUUIDString = ET.SubElement(dictSetting, 'string')
                    payloadUUIDString.text = '50BB7AA6-2489-45FD-A82E-C6E5B5BEB552{0}'.format(e)
            ...
            if policy['type'] == 'vpn':
                ...
                for e, vpnDict in enumerate(vpnDict_list):
                    ...
                    if vpn_type == 'IPSec':
                        ...
                        if vpnDict.has_key('RSASecure') and vpnDict.get('RSASecure'):
                    ...
                    PayloadIdentifierString.text = 'com.toppatch.vpnProfile.vpn{0}'.format(e)
                    ...
                    PayloadUUIDString.text = 'E41B3848-D2F9-4555-BBC3-F6EEFF71690D{0}'.format(e)

                    PayloadVersion = etree.SubElement(dictSetting, 'key')
                    PayloadVersion.text = 'PayloadVersion'

                    PayloadVersionInteger = etree.SubElement(dictSetting, 'integer')
