...
def get_mapped_policy(policy_json):
    ...
    for key in policy_keys:
        ...
        if key == 'access':
            ...
            _restrictions['allow_adding_game_center_friends'] = access_dict.get(
            ...
            _restrictions['allow_global_background_fetch_when_roaming'] = access_dict.get(
                                'allow_global_background_fetch_when_roaming')
            _restrictions['force_itunes_store_password_entry'] = access_dict.get(
    ...
    if install_apps:
        ...
        for apps in iter(install_apps):
            if apps.get('iOS'):
                install_application.append({'id': apps.get('id'),
    ...
    if _restrictions.get('browser_settings'):
        ...
        browser_restriction = _restrictions.get('browser_settings') if ios_flag else None

        if browser_restriction:
            restrictions_data['safariAllowPopups'] = browser_restriction.get(
                                                    'enable_popups')
            restrictions_data['safariAllowJavaScript'] = browser_restriction.get('enable_javascript')
            ...
            restrictions_data['safariForceFraudWarning'] = browser_restriction.get('force_fraud_warnings')

    if _restrictions.get('itunes_enable'):

        ios_flag = _restrictions['itunes_enable'].get('iOS')
        itune_restriction = _restrictions.get('itunes_enable') if ios_flag else None
    ...
    if _restrictions.get('playstore_enable'):
        ...
        playstore_restriction = _restrictions.get('playstore_enable') if ios_flag else None

        if playstore_restriction:
            restrictions_data['allowAppInstallation'] = playstore_restriction.get(
                                                    'value')

    if _restrictions.get('siri_enable'):

        ios_flag = _restrictions['siri_enable'].get('iOS')
        siri_restriction = _restrictions.get('siri_enable') if ios_flag else None
    ...
    if _restrictions.get('safari_enable'):
        ...
        safari_restriction = _restrictions.get('safari_enable') if ios_flag else None
    ...
    if _restrictions.get('in_app_purchases'):
        ...
        in_app_purchases = _restrictions.get('in_app_purchases') if ios_flag else None
    ...
    if _restrictions.get('enable_camera'):
        ...
        camera_restriction = _restrictions.get('enable_camera') if ios_flag else None
    ...
    if _restrictions.get('diagnostic_submission'):
        ...
        report_restriction = _restrictions.get('diagnostic_submission') if ios_flag else None

        if report_restriction:
            restrictions_data['allowDiagnosticSubmission'] = report_restriction.get('value')

    if _restrictions.get('allow_explicit_content'):

        ios_flag = _restrictions['allow_explicit_content'].get('iOS')
        content_restriction = _restrictions.get('allow_explicit_content') if ios_flag else None

        if content_restriction:
            restrictions_data['allowExplicitContent'] = content_restriction.get('value')

    if _restrictions.get('allow_untrusted_tls_prompt'):

        ios_flag = _restrictions['allow_untrusted_tls_prompt'].get('iOS')
        tls_restriction = _restrictions.get('allow_untrusted_tls_prompt') if ios_flag else None

        if tls_restriction:
            restrictions_data['allowUntrustedTLSPrompt'] = tls_restriction.get('value')

    if _restrictions.get('screenshot'):

        ios_flag =  _restrictions['screenshot'].get('iOS')
        screenshot_restriction = _restrictions.get('screenshot') if ios_flag else None
    ...
    if _restrictions.get('allow_cloud_backup'):
        ...
        backup_restriction = _restrictions.get('allow_cloud_backup') if ios_flag else None
    ...
    if _restrictions.get('allow_cloud_document_sync'):
        ...
        sync_restriction = _restrictions.get('allow_cloud_document_sync') if ios_flag else None
    ...
    if _restrictions.get('allow_adding_game_center_friends'):
        ...
        if game_center_restriction:
            restrictions_data['allowAddingGameCenterFriends'] = game_center_restriction.get('value')
    ...
    if _restrictions.get('allow_multi_player_gaming'):
        ...
        if multiplayer_restriction:
            restrictions_data['allowMultiplayerGaming'] = multiplayer_restriction.get('value')
    ...
    if _restrictions.get('allow_photo_stream'):
        ...
        if photo_stream_restriction:
            restrictions_data['allowPhotoStream'] = photo_stream_restriction.get('value')
    ...
    if _restrictions.get('allow_voice_dialing'):
        ...
        if voice_dialing_restriction:
            restrictions_data['allowVoiceDialing'] = voice_dialing_restriction.get('value')
    ...
    if _restrictions.get('allow_video_conferencing'):
        ...
        if conferencing_restriction:
            restrictions_data['allowVideoConferencing'] = conferencing_restriction.get(
                                                                    'value')
    ...
    if _restrictions.get('force_encrypted_backups'):
        ...
        if force_encrypted_backups:
            restrictions_data['forceEncryptedBackup'] = force_encrypted_backups.get(
                                                                    'value')

    if _restrictions.get('allow_global_background_fetch_when_roaming'):

        ios_flag = _restrictions['allow_global_background_fetch_when_roaming'].get('iOS')
    ...
    if _restrictions.get('force_itunes_store_password_entry'):
        ...
        ios_flag = _restrictions['force_itunes_store_password_entry'].get('iOS')
