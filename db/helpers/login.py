...
class LoginDBHelper(DBHelper):
    ...
    def get_login(self, username, update_flag=None):
        ...
        if self.cursor.rowcount > 0:
            ...
            self.log.e(TAG,'Not able to perform select operation on Login table')
            return None
    ...
    def get_login_name(self, id):
        ...
        try:
            self.cursor.execute("SELECT " + C.LOGIN_TABLE_USERNAME + " FROM " + C.LOGIN_TABLE + " WHERE " + C.LOGIN_TABLE_ID + " = " + str(id))
        except Exception, err:
            self.log.e(TAG, 'Exception: ' + repr(err))
            return None
