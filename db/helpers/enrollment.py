...
class EnrollmentDBHelper(DBHelper):
    ...
    def add_enrollment(self, enrollment):
        ...
        if isinstance(enrollment, dict):
            if C.ENROLLMENT_TABLE_USER in enrollment:
                if C.ENROLLMENT_TABLE_PASSWORD in enrollment:
                    if C.ENROLLMENT_TABLE_IS_ENROLLED in enrollment:
                        try:
                            self.cursor.execute(
                                "SELECT * FROM " + C.ENROLLMENT_TABLE +
                                " WHERE " + C.ENROLLMENT_TABLE_IS_ENROLLED
                                + " = False AND " + C.ENROLLMENT_TABLE_USER
                                + " = " +
                                str(enrollment[C.ENROLLMENT_TABLE_USER]))
                        ...
                        if self.cursor.rowcount > 0:
                            self.log.i(TAG, str(
                                ...
                                + " exist already sending the id of \
                                    that user")
                            ...
                            try:
                                self.cursor.execute(
                                    "INSERT INTO " + C.ENROLLMENT_TABLE +
                            ...
                            if self.cursor.rowcount > 0:
                                ...
                                self.log.e(TAG, 'Not able to insert in \
                                               enrollment table')
                                return None
                    else:
                        self.log.e(TAG, 'IS_ENrolled field not found \
                                       in dictionary')
                        return None
                else:
    ...
    def get_enrollment(self, id, pluck=None):
        ...
        if isinstance(id, str):
            if pluck is None:
                try:
                    self.cursor.execute(
                        "SELECT * FROM " + C.ENROLLMENT_TABLE + " WHERE " +
                ...
                try:
                    self.cursor.execute(
                        "SELECT " + query_var + " FROM " + C.ENROLLMENT_TABLE
    ...
    def update_enrollment(self, id, update_dict):
        ...
        if isinstance(id, str) and isinstance(update_dict, dict):
            query_str = ''
            ...
            try:
                self.cursor.execute(
                    "UPDATE " + C.ENROLLMENT_TABLE + " SET " + query_str +
                    " WHERE " + C.ENROLLMENT_TABLE_ID + " = " + str(id))
    ...
    def delete_enrollment(self, id):
        ...
        if isinstance(id, str):
            try:
                self.cursor.execute(
                    "DELETE FROM " + C.ENROLLMENT_TABLE + " WHERE " +
        ...
        if filter_dict is None:
            ...
            if status:
                try:
                    self.cursor.execute(
                        "SELECT *  FROM " + C.ENROLLMENT_TABLE + " WHERE " +
                        query_string + ' AND ' + C.ENROLLMENT_TABLE_IS_ENROLLED
                        + '=' + str(status))
                except Exception as err:
                    self.log.e(TAG, 'Exception: ' + repr(err))
                    return None
    ...
    def get_enrollment_status_count(self, company_id, status):
        ...
        try:
            self.cursor.execute("""SELECT COUNT(*)  FROM enrollments
                        ...
                        AND enrollments.is_enrolled={1};""".format(company_id,
                                                                   status))
        except Exception as err:
            self.log.e(TAG, 'Exception: ' + repr(err))
            return None
...
Enrollment table')
    ...
    def get_enrolled(self, page=None, count=None):
        ...
        zip_list = list(
            self.enroll_tuples).remove(C.ENROLLMENT_TABLE_IS_ENROLLED)

        if page is None or count is None:
            try:
                self.cursor.execute(
                    "SELECT *  FROM " + C.ENROLLMENT_TABLE + " WHERE " +
                    C.ENROLLMENT_TABLE_IS_ENROLLED + " = True")
...
Enrollment table')
            ...
            try:
                self.cursor.execute(
                    "SELECT *  FROM " + C.ENROLLMENT_TABLE + " WHERE " +
                    C.ENROLLMENT_TABLE_IS_ENROLLED + " = True ORDER BY " +
                    str(sort_by) + " LIMIT " + str(limit) + " OFFSET " +
                    str(skip))
...
Enrollment table')
            ...
            try:
                self.cursor.execute(
                    "SELECT *  FROM " + C.ENROLLMENT_TABLE + " WHERE " +
                    C.ENROLLMENT_TABLE_IS_ENROLLED + " = False ORDER BY " +
                    str(sort_by) + " LIMIT " + str(limit) + " OFFSET " +
                    str(skip))
