...
class SessionDBHelper(DBHelper):
    ...
    def __init__(self):
        ...
        self.session_tuple = (C.SESSION_TABLE_ID, C.SESSION_TABLE_CREATED_ON,
                    C.SESSION_TABLE_DESTROYED_ON, C.SESSION_TABLE_IP,
                    C.SESSION_TABLE_INVALID, C.SESSION_TABLE_USER_AGENT,
    ...
    def add_session(self,session):
        ...
        if type(session) == dict:
            if session.has_key(C.SESSION_TABLE_IP) and session.has_key(C.SESSION_TABLE_USER) and session.has_key(C.SESSION_TABLE_USER_AGENT):
                key_str = ''
                value_str = ''
                for key, value in session.iteritems():
                    key_str += str(key) + ","
                    if str(key) == C.SESSION_TABLE_IP or str(key) == C.SESSION_TABLE_USER or str(key) == C.SESSION_TABLE_USER_AGENT or str(key) == C.SESSION_TABLE_CREATED_ON or str(key) == C.SESSION_TABLE_DESTROYED_ON:
                        value_str += "'" + str(value) + "',"
                    else:
                        value_str += str(value) + ","
                ...
                try:
                    self.cursor.execute("INSERT INTO " + C.SESSION_TABLE + "(" + str(key_str) + "," + C.SESSION_TABLE_CREATED_ON + "," + C.SESSION_TABLE_INVALID + ") VALUES(" + str(value_str) + ",'" + str(datetime.datetime.now()) + "', False) RETURNING id")
                ...
                self.log.e(TAG, 'IP/UserID/User Agent is not sent in insertion dictionary')
    ...
    def get_session(self,id,pluck=None):
        ...
        if type(id) == str:
            if pluck is None:
                try:
                    self.cursor.execute("SELECT * FROM " + C.SESSION_TABLE + " WHERE " + C.SESSION_TABLE_ID + " = " + id)
                except Exception, err:
                    self.log.e(TAG, 'Exception: ' + repr(err))
                    return None
                ...
                if self.cursor.rowcount > 0:
                    ...
                    self.log.e(TAG,'Not able to perform select operation on Session Table')
                    return None
                ...
                try:
                    self.cursor.execute("SELECT " + query_var + " FROM " + C.SESSION_TABLE + " WHERE " + C.SESSION_TABLE_ID + " = " + id)
                except Exception, err:
                    self.log.e(TAG, 'Exception: ' + repr(err))
                    return None
    ...
    def destroy_session(self,id):
        ...
        if type(id) == str:
            print "UPDATE " + C.SESSION_TABLE + " SET " + C.SESSION_TABLE_INVALID + " = True AND " + C.SESSION_TABLE_DESTROYED_ON + " = '" + str(datetime.datetime.now()) + "' WHERE " + C.SESSION_TABLE_ID + " = " + str(id)
            try:
                self.cursor.execute("UPDATE " + C.SESSION_TABLE + " SET " + C.SESSION_TABLE_INVALID + " = True, " + C.SESSION_TABLE_DESTROYED_ON + " = '" + str(datetime.datetime.now()) + "' WHERE " + C.SESSION_TABLE_ID + " = " + str(id))
            except Exception, err:
                self.log.e(TAG, 'Exception: ' + repr(err))
                return False
    ...
    def validate_session(self,id,ip,user_agent):
        session=self.get_session(id, [C.SESSION_TABLE_IP,C.SESSION_TABLE_USER_AGENT])
        if session[C.SESSION_TABLE_IP] == ip and session[C.SESSION_TABLE_USER_AGENT]== user_agent:
            return True
        else:
            return False
...
#     print session.add_session(session_dict)
    ...
    print session.get_session('9', [C.SESSION_TABLE_CREATED_ON,C.SESSION_TABLE_USER_AGENT])
