...
class LogsDBHelper(DBHelper):
    ...
    def add_log(self, component_id, component, level, tag, message, raw=None,
                company='1'):
        TAG = 'add_log'

        if (component_id is not None and component is not None and
                level is not None and message is not None):
            try:
                self.cursor.execute(
                    "INSERT INTO " + C.LOGS_TABLE + "(" +
                    C.LOGS_TABLE_COMPONENT_TYPE + "," +
                    C.LOGS_TABLE_COMPONENT_ID + "," + C.LOGS_TABLE_LEVEL +
                    "," + C.LOGS_TABLE_TAG + "," + C.LOGS_TABLE_MESSAGE + ","
                    + C.LOGS_TABLE_RAW + "," + C.LOGS_TABLE_COMPANY + "," +
                    C.LOGS_TABLE_TIMESTAMP +
                    ") VALUES( %s,%s,%s,%s,%s,%s,%s,%s) RETURNING id",
                    [component, component_id, level, tag, message,
                        raw, company, datetime.datetime.now()])
    ...
    def get_logs(self, company, levels=None, count=None):
        ...
        if company is not None:
            if levels is None:
                try:
                    self.cursor.execute(
                        "SELECT * FROM " + C.LOGS_TABLE + " WHERE " +
                ...
                level_string = ''
                for level in levels:
                    level_string += (C.LOGS_TABLE_LEVEL + " = '" +
                                     str(level) + "' OR ")
                ...
                try:
                    self.cursor.execute(
                        "SELECT * FROM " + C.LOGS_TABLE + " WHERE " +
                        str(level_string) + " AND " + C.LOGS_TABLE_COMPANY +
                        " = " + str(company) + " ORDER BY " +
                        C.LOGS_TABLE_TIMESTAMP + " DESC LIMIT " + str(limit))
