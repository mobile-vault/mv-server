...
class UserDBHelper(DBHelper):
    ...
    def is_user_valid(self, user_id):
        ...
        if isinstance(user_id, str):
            try:
                self.cursor.execute("""SELECT * FROM {0} WHERE {1} = False
                    AND {2} = {3};""".format(C.USER_TABLE,
                    C.USER_TABLE_DELETED, C.USER_TABLE_ID, user_id))
    ...
    def add_user_if_not_exists(self,user):
        ...
        if isinstance(user, dict):
            if user.has_key(C.USER_TABLE_EMAIL) and \
                user.has_key(C.USER_TABLE_NAME) and \
                user.has_key(C.USER_TABLE_COMPANY):
                from validate_email import validate_email
                if validate_email(user[C.USER_TABLE_EMAIL]):
                    try:
                        self.cursor.execute("SELECT * FROM " + C.USER_TABLE +\
                             " WHERE " + C.USER_TABLE_DELETED+" = False AND "
                             + C.USER_TABLE_EMAIL + " = '" +\
                    ...
                    if self.cursor.rowcount > 0:
                        self.log.i(TAG, str(user[C.USER_TABLE_NAME]) +\
                        ...
                        print "INSERT INTO " + C.USER_TABLE + "(" \
                            + str(key_str) + ") VALUES(" + str(value_str) + ")\
                                 RETURNING id"
                        try:
                            self.cursor.execute("INSERT INTO " + C.USER_TABLE\
                                 + "(" + str(key_str) + "," + \
                                C.USER_TABLE_DELETED  +") VALUES("\
                                 + str(value_str) + ", False) RETURNING id")
                            ...
                            self.cursor.execute(""" UPDATE users
                                ...
                                RETURNING id
                                """.format(adapt(user.get('name')),
                                user.get('role_id'), user.get('team_id'),
                                user.get('company_id'),
                                'email', adapt(user.get('email'))))
            ...
            if user.has_key(C.USER_TABLE_EMAIL) and\
                ...
                if validate_email(user[C.USER_TABLE_EMAIL]):
                    ...
                    if self.cursor.rowcount > 0:
                        ...
                        try:
                            self.cursor.execute("INSERT INTO " + C.USER_TABLE+\
                             " (" + str(key_str) + "," + C.USER_TABLE_DELETED\
                               +") VALUES(" + str(value_str) +\
                                ", False) RETURNING id")
                        except Exception,err:
                            self.log.e(TAG,'Exception : ' + repr(err))
                            return None
    ...
    def get_user_with_email(self,email):
        ...
        if email is not None:
            try:
                self.cursor.execute("SELECT * FROM users WHERE deleted=FALSE AND EMAIL='{0}'".format(email))
    ...
    def update_user(self, user_id, user_dict):
        ...
        if isinstance(user_id, str) and isinstance(user_dict, dict):
            try:
                self.cursor.execute("""UPDATE {0} SET {1}='{2}', {3}={4},
                                 {5}={6} WHERE {7} = False AND {8} = {9}
                                 ;""".format(C.USER_TABLE, C.USER_TABLE_NAME,
                                user_dict.get(C.USER_TABLE_NAME),
                                C.USER_TABLE_TEAM,
                                user_dict.get(C.USER_TABLE_TEAM),
                                C.USER_TABLE_ROLE, user_dict.get(
        ...
        if isinstance(user_id, str) and isinstance(role_id, str):
            try:
                self.cursor.execute("UPDATE " + C.USER_TABLE + " SET "\
                    + C.USER_TABLE_ROLE + " = " + str(role_id) + " WHERE "\
    ...
    def update_user_team(self, user_id, team_id):
        ...
        if isinstance(user_id, str) and isinstance(team_id, str):
            try:
                self.cursor.execute("UPDATE " + C.USER_TABLE + " SET "\
                     + C.USER_TABLE_TEAM + " = " + str(team_id) + " WHERE "\
                        + C.USER_TABLE_DELETED+" = False AND "\
                      + C.USER_TABLE_ID + " = " + str(user_id))
            except Exception, err:
                    self.log.e(TAG, 'Exception: ' + repr(err))
    ...
    def update_user_policy(self, user_id, policy_id):
        ...
        if isinstance(user_id, str) and isinstance(policy_id, str):
            try:
                self.cursor.execute("UPDATE " + C.USER_TABLE + " SET " +\
                         C.USER_TABLE_POLICY + " = " + str(policy_id) +\
                        " WHERE " + C.USER_TABLE_DELETED+" = False AND " \
                         + C.USER_TABLE_ID + " = " + str(user_id))
            except Exception, err:
                    self.log.e(TAG, 'Exception: ' + repr(err))
    ...
    def update_user_name(self,id,name):
        ...
        if type(id) == str and type(name) == str:
            try:
                self.cursor.execute("UPDATE " + C.USER_TABLE + " SET "\
                     + C.USER_TABLE_NAME + " = '" + str(name) + "' WHERE " + \
                        C.USER_TABLE_ID + " = " + str(id))
            except Exception, err:
                    self.log.e(TAG, 'Exception: ' + repr(err))
                    return False
    ...
    def get_users(self,filter_dict=None):
        ...
        if isinstance(filter_dict, dict) and filter_dict.has_key('company_id'):
            query_string = ''
            for key, value in filter_dict.iteritems():
                if str(key) == C.USER_TABLE_EMAIL or str(key) == C.USER_TABLE_NAME:
                    query_string += 'users.'+str(key) + " = '" + str(value) + "' AND "
                else:
                    query_string += 'users.'+str(key) + " = " + str(value) + ","
            ...
            if self.cursor.rowcount > 0:
                ...
                mapping_tuple = ('user_device', 'user_id', 'user_email',
                     'user_device_os', 'user_name', 'user_role', 'user_team')
                ...
                self.log.e(TAG,'No user found over select operation on User table')
                return None, total_count
        else:
    ...
    def get_users_with_pages(self, filter=None, page=None, count=None,
        ...
        if type(filter) == dict:
            ...
            try:
                self.cursor.execute("SELECT *  FROM " + C.USER_TABLE +\
                    " WHERE " + query_string + " ORDER BY " + str(sort_by) +\
            ...
            try:
                self.cursor.execute("SELECT *  FROM " + C.USER_TABLE +\
                        " ORDER BY " + str(sort_by) + " LIMIT "+str(limit)+\
                        " OFFSET " + str(skip))
            except Exception, err:
                self.log.e(TAG, 'Exception: ' + repr(err))
                return None
    ...
    def get_users_count(self, company_id):
        TAG = 'get_users'
        try:
            self.cursor.execute("SELECT COUNT(*)  FROM " + C.USER_TABLE+\
                    " WHERE "+ C.USER_TABLE_COMPANY + "="+ str(company_id)\
                    + " AND " + C.USER_TABLE_DELETED + " = False;")
        except Exception, err:
            self.log.e(TAG, 'Exception: ' + repr(err))
            return None
        ...
        if self.cursor.rowcount > 0:
            ...
            self.log.e(TAG,'Not able to perform select operation on User table')
            return None
    ...
    def get_users_for_team(self, team_name, team_id, role_name=None,
            role_id=None, offset=None, count=None, sort_by=None, query=None,
            query_type=None, sort_order=None):
    ...
    def get_users_base_helper(self, company_id, final_query,final_query_total,
                    offset=None, count=None,  query=None, query_type=None,
                    role_id=None, team_id=None, os_type=None, sort_by=None,
                    sort_order=None, filter_key=None, filter_value=None):
    ...
    def get_users_for_user(self, company_id, offset, count, query=None,
        ...
        final_list, total_count = self.get_users_base_helper(company_id=company_id,
                    offset=offset, count=count, final_query=final_query,
                    final_query_total=final_query_total, query=query,
                    query_type=query_type, role_id=role_id, team_id=team_id,
                    os_type=os_type, sort_by=sort_by, sort_order=sort_order,
                    filter_key=filter_key, filter_value=filter_value)

        return final_list, total_count


    def get_users_for_user_status(self,company_id, offset, count, data,
                        query=None, query_type=None, role_id=None,
                        team_id=None, os_type=None, sort_by=None,
                        sort_order=None, filter_key=None, filter_value=None):
    ...
    def get_users_for_user_violation(self,company_id, offset, count,
                        query=None, query_type=None, role_id=None,
                        team_id=None, os_type=None, sort_by=None,
                        sort_order=None, filter_key=None, filter_value=None):
        ...
        base_query_total = """SELECT COUNT(DISTINCT users.id) FROM users
                            ...
                            INNER JOIN roles ON users.role_id = roles.id
                            INNER JOIN teams ON users.team_id = teams.id
                            INNER JOIN violations ON devices.id = violations.device_id
                            WHERE users.company_id={0} AND users.deleted=False
                            AND roles.deleted=False
                            AND teams.deleted=False AND violations.deleted=False
                            """
