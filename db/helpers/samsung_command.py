...
class SamsungCommandsDBHelper(DBHelper):
    ...
    def get_commands(
        ...
        try:
            where_clause = ''
            if uuid is not None:
                where_clause = C.SAMSUNG_COMMANDS_TABLE_UUID + \
                    " = '" + str(uuid) + "'"
            if executed is not None:
                if not where_clause:
                    where_clause = C.SAMSUNG_COMMANDS_TABLE_EXECUTED + \
                        ' = ' + str(executed)
                else:
                    where_clause = where_clause + ' and ' + \
                        C.SAMSUNG_COMMANDS_TABLE_EXECUTED + ' = ' + str(
                            executed)
            if device_id is not None:
                if not where_clause:
                    where_clause = C.SAMSUNG_COMMANDS_TABLE_DEVICE + \
                        '=' + str(device_id)
                else:
                    where_clause = where_clause + ' and ' + \
                        C.SAMSUNG_COMMANDS_TABLE_DEVICE + '=' + str(device_id)
    ...
    def get_command(self, command_id, pluck=None):
        ...
        if command_id is not None:
            ...
            print("SELECT * FROM " + C.SAMSUNG_COMMANDS_TABLE + " WHERE " +
                  C.SAMSUNG_COMMANDS_TABLE_ID + "= " + str(command_id))

            self.cursor.execute(
                "SELECT * FROM " +
                C.SAMSUNG_COMMANDS_TABLE +
                " WHERE " +
                C.SAMSUNG_COMMANDS_TABLE_ID +
                "= " +
                str(command_id))
        ...
        if isinstance(command, dict):
            if command.get(C.SAMSUNG_COMMANDS_TABLE_ACTION) and \
               command.get(C.SAMSUNG_COMMANDS_TABLE_DEVICE) \
                ...
                self.cursor.execute(
                    """ insert into samsung_commands
                                    (device_id, action,attribute,command_uuid)
                                    values( %s,%s,%s,%s) returning id""", [
                    command[
                        C.SAMSUNG_COMMANDS_TABLE_DEVICE], command[
                        C.SAMSUNG_COMMANDS_TABLE_ACTION], adapt(
                        command[C.SAMSUNG_COMMANDS_TABLE_ATTRIBUTE]), adapt(
                        command[C.SAMSUNG_COMMANDS_TABLE_UUID])])
            ...
            self.log.i(
                TAG,
                'Not all required parameters sent in ' +
                command.__repr__())
            return None
    # BIGNET299
    ...
    def update_result(self, uuid, device_id, result):
        ...
        if isinstance(uuid, str) and isinstance(device_id, str):
            try:
                self.cursor.execute(
                    """UPDATE samsung_commands  SET {0} = {1},
    ...
    def set_executed(self, command_id, result=None):
        ...
        if command_id is not None:
            if not result:
                query = (
                    "UPDATE " + C.SAMSUNG_COMMANDS_TABLE + " SET " +
                    C.SAMSUNG_COMMANDS_TABLE_EXECUTED + " = true " +
                    " WHERE " + C.SAMSUNG_COMMANDS_TABLE_ID + "=" +
                    str(command_id))
                ...
                query = (
                    "UPDATE " + C.SAMSUNG_COMMANDS_TABLE + " SET " +
                    C.SAMSUNG_COMMANDS_TABLE_EXECUTED + " = true , " +
                    C.SAMSUNG_COMMANDS_TABLE_RESULT + "= '" + str(result) +
                    "' WHERE " + C.SAMSUNG_COMMANDS_TABLE_ID + "=" +
                    str(command_id))
                print query
