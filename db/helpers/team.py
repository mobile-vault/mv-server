...
class TeamDBHelper(DBHelper):
    ...
    def add_team(self, team):
        ...
        if isinstance(team, dict):
            if C.TEAM_TABLE_NAME in team and C.TEAM_TABLE_COMPANY in team:
                try:
                    ...
                    try:
                        self.cursor.execute(
                            """INSERT INTO {0} ({1}, {2}, {3})
                                    ...
                                    """.format(
                            C.TEAM_TABLE, C.TEAM_TABLE_NAME,
                            C.TEAM_TABLE_COMPANY, C.TEAM_TABLE_DELETED),
                            (team.get(C.TEAM_TABLE_NAME),
                                int(team.get(C.TEAM_TABLE_COMPANY)), team[
                                    C.TEAM_TABLE_DELETED]))
    ...
    def update_team(self, team_id, company_id, team):
        ...
        if isinstance(team_id, str) and isinstance(team, dict) and company_id:
            query_str = ''
            for key, value in team.iteritems():
                if str(key) == C.TEAM_TABLE_NAME:
