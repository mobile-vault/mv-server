...
class DeviceDeleteHandlerThread(threading.Thread):
    ...
    def run(self):
        ...
        if self.data is None:
            ...
            opJson = json.dumps(
                {'pass': False, 'message': 'No Device ID in the request'})
            ...
            if not is_deleted:
                log.e(
                    TAG, 'Not able to deleted the device id = ' + str(
                        self.data))
                opJson = json.dumps(
                    {'pass': False,
                        'message': 'Not able to deleted the device id = \
' + str(self.data)})
                #self.request.add_header('Access-Control-Allow-Origin', '*')
                self.request.set_header('Content-Type', 'application/json')
                self.request.write(opJson)
                ...
                opJson = json.dumps(
                    {'pass': True, 'message': 'Device Deleted Successfully'})
                #self.request.add_header('Access-Control-Allow-Origin', '*')
                self.request.set_header('Content-Type', 'application/json')
                self.request.write(opJson)
                tornado.ioloop.IOLoop.instance().add_callback(self.callback)
...
class DeviceGetHandlerThread(threading.Thread):
    ...
    def run(self):
        ...
        device = DeviceDBHelper()
        device_dict = device.get_device(
            str(self.data), [c.DEVICE_TABLE_OS, c.DEVICE_TABLE_USER])
        if device_dict is None:
            log.e(
                TAG, 'No device data corresponding to this device_id = ' + str(
                    self.data))
            opJson = json.dumps(
                {'pass': False,
                    'message': 'No device data corresponding to this device \
Id = ' + str(self.data)})
            #self.request.add_header('Access-Control-Allow-Origin', '*')
            self.request.set_header('Content-Type', 'application/json')
            self.request.write(opJson)
            ...
            if self.device_os is None or self.device_os is None:
                log.e(
                    TAG,
                    'Either device os or User id is nOne for device id = \
' + str(self.data))

                opJson = json.dumps(
                    {'pass': False,
                        'message': 'Either device os or User id is nOne \
for device id = ' + str(self.data)})
                #self.request.add_header('Access-Control-Allow-Origin', '*')
                self.request.set_header('Content-Type', 'application/json')
                self.request.write(opJson)
                tornado.ioloop.IOLoop.instance().add_callback(self.callback)
                ...
                if user_dict is None:
                    log.e(
                        TAG,
                        'No user info for the user id = ' +
                        self.user_id)
                    opJson = json.dumps(
                        {'pass': False,
                            'message': 'No user info for the user id = \
' + self.user_id})
                    self.request.set_header('Content-Type', 'application/json')
                    self.request.write(opJson)
                    ...
                    if user_role_id is not None:
                        ...
                        if role_dict is None:
                            log.e(
                                TAG,
                                'No role in role table corresponding to \
the role_id = ' +
                                str(user_role_id))
                            opJson = json.dumps(
                                {
                                    'pass': False,
                                    'message': 'No role in role table \
corresponding to the role_id = ' +
                                    str(user_role_id)})
                    ...
                    if user_team_id is not None:
                        ...
                        if team_dict is None:
                            log.e(
                                TAG,
                                'No team in team table corresponding to \
the team_id = ' +
                                str(user_team_id))
                            opJson = json.dumps(
                                {
                                    'pass': False,
                                    'message': 'No team in team table \
corresponding to the team_id = ' + str(user_team_id)})
                            self.request.set_header(
                                'Content-Type',
                                'application/json')
                            self.request.write(opJson)
...
class DevicesGetHandlerThread(threading.Thread):
    ...
    def run(self):
        ...
        device = DeviceDBHelper()
        device_list = device.get_devices_with_pages(
            None,
            int(page),
            int(count),
            str(sort_by))
        if device_list is None:
            ...
            opJson = json.dumps(
                {'pass': True, 'message': 'No Device registered'})
            #self.request.add_header('Access-Control-Allow-Origin', '*')
            self.request.set_header('Content-Type', 'application/json')
            self.request.write(opJson)
            tornado.ioloop.IOLoop.instance().add_callback(self.callback)
        ...
        for devices in device_list:
            ...
            if self.user_id is None or self.device_id is None:
                log.e(
                    TAG,
                    'No user id corresponding to the device id = ' + str(
                        self.device_id))
                opJson = json.dumps(
                    {'pass': False,
                        'message': 'No user id corresponding to the \
device id = ' + str(self.device_id)})
                ...
                user = UserDBHelper()
                user_list = user.get_user(str(self.user_id),
                                          [c.USER_TABLE_ROLE,
                                           c.USER_TABLE_TEAM,
                                           c.USER_TABLE_NAME,
                                           c.USER_TABLE_EMAIL])
                ...
                if user_list is None:
                    log.e(
                        TAG,
                        'No user details corresponding to the userid = ' + str(
                            self.user_id))
                    opJson = json.dumps(
                        {'pass': False,
                            'message': 'No user details corresponding to \
the user id = ' + str(self.user_id)})
                    self.request.set_header('Content-Type', 'application/json')
                    self.request.write(opJson)
                    tornado.ioloop.IOLoop.instance().add_callback(
                        self.callback)
