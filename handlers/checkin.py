...
class CheckinPerformerThread(threading.Thread):
    ...
    udid = 'UDID'
    do_entry = False
    def __init__(self, data=None,callback=None,request = None, *args, **kwargs):
...
#         tree = ET.parse('temp.xml')   ### For testing only
        ...
        if self.do_entry:
            ...
            if enrollment_dict is None:
                ...
                devices = device.get_device_with_udid(str(self.udid), status=True)
                ...
                if device_id:
                    ...
                    device_detail.update_device_details(str(device_id),
                                                device_details_dict)
                    enrollment.update_enrollment(enrollment_id,
                    ...
                    if device_id is None:
                        ...
                        device_details_dict_new[c.DEVICE_DETAILS_TABLE_DEVICE] = device_id
                        device_details_dict_new[c.DEVICE_DETAILS_TABLE_EXTRAS] = device_details_dict
                        ...
                        if device_details_id is None:
                            log.e(TAG,'Not Able to insert in Device Details \
                                  table UDID = ' + str(self.udid) + 'userID = '\
                                + str(user_id) + 'DeviceID = '+ str(device_id))
                        else:
                            ...
                            if not success:
                                ...
                                success1 = enrollment.set_enrolled(enrollment_id)

                                if success1:
                                    enrolled_success = True
                                else:
                                    log.e(TAG, 'EnrolledOn time is not updated in the Enrollment Table')
                ...
                if device_id and enrolled_success:
                    ...
                    if violation_status:
                        ...
                        log.e(TAG,
                            "Violation table not updated for device_id"+ str(
                            device_id))
