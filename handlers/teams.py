...
class TeamGetHandlerThread(threading.Thread):
    ...
    def run(self):
        ...
        if self.data is None:
            ...
            opJson = json.dumps(
                {'pass': False,
                    'message': 'No team found corresponding to the team \
id ' + str(self.data)})
            self.request.write(opJson)
            tornado.ioloop.IOLoop.instance().add_callback(self.callback)
        else:
            ...
            if teams is None:
                log.e(
                    TAG,
                    'No team found corresponding to the team id ' + str(
                        self.data))
                opJson = json.dumps(
                    {'pass': False,
                        'message': 'No team found corresponding to the team \
        ...
        if self.team_id is None:
            ...
            if 'name' in input_dict:
                ...
                if self.team_name is None:
                    ...
                    opJson = json.dumps(
                        {'pass': False, 'message': 'No team_name sent'})
                    self.request.write(opJson)
...
team_name at DB'})
                        ...
                        opJson = json.dumps(
                            {'pass': True,
                                'message': 'team_name updated for team.'})
                        self.request.write(opJson)
                        tornado.ioloop.IOLoop.instance().add_callback(
                            self.callback)
                ...
                if self.team_policy_id is None:
                    ...
                    opJson = json.dumps(
                        {'pass': False, 'message': 'No team_policy_id sent'})
                    self.request.write(opJson)
                    tornado.ioloop.IOLoop.instance().add_callback(
                        self.callback)
                else:
...
team_policy_id at DB'})
                        ...
                        opJson = json.dumps(
                            {'pass': True,
                                'message': 'team_policy_id updated.'})
                        self.request.write(opJson)
                        tornado.ioloop.IOLoop.instance().add_callback(
                            self.callback)
    ...
    def run(self):
        ...
        if self.data is None:
            ...
            if is_deleted:
                ...
                opJson = json.dumps(
                    {'pass': True, 'message': 'Team Successfully deleted'})
                self.request.write(opJson)
                tornado.ioloop.IOLoop.instance().add_callback(self.callback)
            else:
                log.e(TAG, 'Team Not Deleted')
                opJson = json.dumps(
                    {'pass': False, 'message': 'Team Not Deleted'})
                self.request.write(opJson)
                tornado.ioloop.IOLoop.instance().add_callback(self.callback)
