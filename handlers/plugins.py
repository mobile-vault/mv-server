...
def put_individual_plugin(policy_id, plugin_name, plugin_data):
    ...
    policy_dict[C.POLICY_TABLE_OLD_ATTRIBUTES] = policy_dict[C.POLICY_TABLE_NEW_ATTRIBUTES]
    if plugin_data.has_key('_id'):
        plugin_data.pop('_id')
    if plugin_data.has_key('name'):
        plugin_data.pop('name')
    if plugin_data.has_key('object_type'):
...
def setup_default_policy():
    ...
    if policy_id:
        ...
        print """Something went wrong with database policy not saved into policy
        table """
